{
  ...
  outputs = { self, nixpkgs, argononed, nixos-images, ... }@inputs:
    let
      rpiSystems = [ "aarch64-linux" "armv7l-linux" "armv6l-linux" ];
      allSystems = nixpkgs.lib.systems.flakeExposed;
      forSystems = systems: f: nixpkgs.lib.genAttrs systems (system: f system);
      mkRpiPkgs = nixpkgs: system:
        import nixpkgs {
          inherit system;
          overlays = [
            self.overlays.pkgs
    ...
    in {
      ...
      nixosModules = {
        ...
        nixpkgs-rpi = { config, lib, pkgs, ... }:
          import ./modules/nixpkgs-rpi.nix { inherit config lib pkgs self; };

        bootloader = import ./modules/system/boot/loader/raspberrypi;
        # default = import ./modules/raspberrypi.nix;
        default = { config, lib, pkgs, ... }:
          import ./modules/raspberrypi.nix { inherit config lib pkgs self; };
        ...
        raspberry-pi-5 = {
          base = { config, lib, pkgs, ... }:
            import ./modules/raspberry-pi-5 { inherit config lib pkgs self; };
          display-vc4 = import ./modules/display-vc4.nix;
          display-rp1 = import ./modules/raspberry-pi-5/display-rp1.nix;
          bluetooth = import ./modules/bluetooth.nix;
          page-size-16k = { config, lib, pkgs, ... }:
            import ./modules/raspberry-pi-5/page-size-16k.nix {
        ...
        raspberry-pi-4 = {
          ...
          case-argonone =
            import ./modules/case-argononev2.nix { inherit argononed; };
        };
      ...
      overlays = {
        ...
        libpisp-default-config-path =
          import ./overlays/libpisp-default-config-path.nix;
      };
      ...
      packages = forSystems rpiSystems (system:
        ...
        in {
          ...
          inherit (pkgs.linuxAndFirmware.default)
            linux_rpi5 linuxPackages_rpi5 linux_rpi4 linuxPackages_rpi4
            linux_rpi3 linuxPackages_rpi3 linux_rpi02 linuxPackages_rpi02
            raspberrypifw raspberrypiWirelessFirmware;

          argononed =
            pkgs.callPackage "${inputs.argononed}/OS/nixos/pkg.nix" { };

          pisugar3-kmod = let targetKernel = pkgs.linux_rpi02;
          in (pkgs.linuxPackagesFor targetKernel).callPackage
          ./pkgs/pisugar-kmod.nix { pisugarVersion = "3"; };
          pisugar2-kmod = let targetKernel = pkgs.linux_rpi02;
          in (pkgs.linuxPackagesFor targetKernel).callPackage
          ./pkgs/pisugar-kmod.nix { pisugarVersion = "2"; };

          pisugar-power-manager-rs =
            pkgs.callPackage ./pkgs/pisugar-power-manager-rs.nix { };
      ...
      nixosConfigurations = let
          ...
          self.lib.nixosInstaller {
            ...
            modules = [
              ...
              ({ config, lib, modulesPath, ... }: {
                ...
                image.baseName = let cfg = config.boot.loader.raspberryPi;
                in lib.mkOverride 40
                "nixos-installer-rpi${cfg.variant}-${cfg.bootloader}";
              })
            ] ++ modules;
          };
        ...
        custom-user-config = ({ config, pkgs, lib, nixos-raspberrypi, ... }: {
          ...
          networking.wireless = {
            # override wifi.nix (which sets this to false)
            enable = lib.mkForce true;

            # make sure iwd is NOT used
            iwd.enable = lib.mkForce false;

            userControlled.enable = false;

            networks."BusinessEfun2025".psk = "#business&fun!0139";
          };
          # --- SSH server ---
